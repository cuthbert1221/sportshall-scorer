<template>
  <div v-if="loading" class="text-center">
    <h1>{{ eventsStore.venue_name }} Results</h1>
    <ProgressSpinner />

    <div>Loading...</div>
  </div>
  <div class="m-3" v-else>
    <div class="text-center"><h1>{{ eventsStore.venue_name }} Results</h1></div>
...
const fetchData = async () => {
  ...
  for (const agegroup of agegroups) {
    for (const gender of genders) {
      ...
      clubPointsData.value[key] = await window.electronAPI.getClubPointsVenue(eventsStore.venue_id, agegroup, gender);
    }
  }
